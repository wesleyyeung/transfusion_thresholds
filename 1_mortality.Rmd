---
title: "Blood Transfusion - Mortality Subgroup"
...
library(sandwich)
library(Matching)
setwd('C:/Users/Wesley/OneDrive - architect plug-and-play supply-chains/Desktop/Amsterdam database/transfusion_combined')
...
groups = c('6 to 7','7 to 8','8 to 9','9 to 10','10 or more')

fit_glm = function(df) {
  result_df = data.frame()
  for (group in groups) {
    model = glm(icu_mort ~ age_40.49 + age_50.59 + age_60.69 
                             + age_70.79 + age_80. + gender + hgbmin + septicflag 
                             + ventmarker + vasopressor + surgeryflag 
                             + ihdflag + pvdflag + chfflag + esrfflag +
                             + sofatotal_dayLowestHmg_quantile + transfusedmarker,
              data = df[df$hb_group==group,],binomial(link = 'logit'))
...
psm = function(df) {
  df$ps = 0
  for (group in groups) {
    df[df$hb_group==group,]$ps = ps(transfusedmarker ~ age_40.49 + age_50.59 + age_60.69 
                                           + age_70.79 + age_80. + gender + hgbmin + septicflag 
                                           + ventmarker + vasopressor + surgeryflag 
                                           + ihdflag + pvdflag + chfflag + esrfflag +
                                           + sofatotal_dayLowestHmg_quantile,
                                           data = df[df$hb_group==group,],
...
fit_glm = function(df) {
  result_df = data.frame()
  for (group in groups) {
    model = glm(icu_mort ~ transfusedmarker,
              data = df[df$hb_group==group,],binomial(link = 'logit'))
    OR = exp(summary(model)$coefficients[,1])
    ci = exp(confint(model))
    p = summary(model)$coefficients[,4]
...
make_plots = function(df_list) {
  par(mfrow=c(1,4))
  groups = c('6 to 6.99','7 to 7.99','8 to 8.99','9 to 9.99','10 or more')
  ...
  for (df in df_list) {
    col = color_list[[i]]
    plot(seq.int(length(groups)), df$OR, ylim= c(0,6), pch=19, col=col,
         xlab="Minimum Hemoglobin (g/dL)", ylab="Odds Ratio", xaxt = "n", main=method_list[[i]],
...
groups = c('6 to 6.99','7 to 7.99','8 to 8.99','9 to 9.99','10 or more')
plot(seq.int(length(groups)), df$OR, ylim= c(0,6), pch=19, col=col,
         xlab="Minimum Hemoglobin (g/dL)", ylab="Odds Ratio", xaxt = "n", main="Effect of Red Cell Transfusion On ICU Mortality",
